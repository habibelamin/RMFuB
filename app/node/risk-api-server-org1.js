...
app.get('/api/risks', async function (req, res) {
    try {
        ...
        let ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));
        // Create a new file system based wallet for managing identities.
        const walletPath = path.join('../../../vars/profiles/vscode/wallets', 'org1.rm');
        const wallet = await Wallets.newFileSystemWallet(walletPath);
        console.log(`Wallet path: ${walletPath}`);

        // Check to see if we've already enrolled the user.
        const identity = await wallet.get('riskmanager1');
        if (!identity) {
            console.log('An identity for the user "riskmanager1" does not exist in the wallet');
        ...
        await gateway.connect(ccp, { wallet, identity: 'riskmanager1', discovery: { enabled: true, asLocalhost: true } });
...
app.get('/api/risks/:risk_index', async function (req, res) {
    try {
        ...
        const result = await contract.evaluateTransaction('QueryRisk', req.params.risk_index);
        console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
        // res.status(200).json({response: result.toString()});
        res.json(JSON.parse(result.toString()));
...
app.delete('/api/risks/:risk_index', async function (req, res) {
    try {
        ...
        // queryCar transaction - requires 1 argument, ex: ('queryCar', 'CAR4')
        // queryAllCars transaction - requires no arguments, ex: ('queryAllCars')
        await contract.submitTransaction('DeleteRisk', req.params.risk_index);
...
app.post('/api/risks/:risk_index', async function (req, res) {
    try {
        ...
        // createCar transaction - requires 5 argument, ex: ('createCar', 'CAR12', 'Honda', 'Accord', 'Black', 'Tom')
        // changeCarOwner transaction - requires 2 args , ex: ('changeCarOwner', 'CAR10', 'Dave')
        await contract.submitTransaction('EditRisk', req.params.risk_index, req.body.rname, req.body.rdescription, req.body.acceptance, req.body.likelihood, req.body.impact, req.body.assessment.aid);
        const listener = await contract.addContractListener(async (event) => {
...
app.put('/api/risks/:risk_index', async function (req, res) {
    try {
        ...
        // createCar transaction - requires 5 argument, ex: ('createCar', 'CAR12', 'Honda', 'Accord', 'Black', 'Tom')
        // changeCarOwner transaction - requires 2 args , ex: ('changeCarOwner', 'CAR10', 'Dave')
        await contract.submitTransaction('TransferRisk', req.params.risk_index, JSON.stringify(req.body.org));
        const listener = await contract.addContractListener(async (event) => {
...
//         // createCar transaction - requires 5 argument, ex: ('createCar', 'CAR12', 'Honda', 'Accord', 'Black', 'Tom')
//         // changeCarOwner transaction - requires 2 args , ex: ('changeCarOwner', 'CAR10', 'Dave')
//         await contract.submitTransaction('TransferRisk', req.body.risk_index, req.body.info);
//         const listener = await contract.addContractListener(async (event) => {
...
app.post('/api/mitigations', async function (req, res) {
    try {
        ...
        // createCar transaction - requires 5 argument, ex: ('createCar', 'CAR12', 'Honda', 'Accord', 'Black', 'Tom')
        // changeCarOwner transaction - requires 2 args , ex: ('changeCarOwner', 'CAR10', 'Dave')
        // await contract.submitTransaction('CreateMitigation','mit5','helo','riskmanager1','now','RISK1:hi:0.3:0.2;');
...
app.get('/api/monitor/securitycontrols', async function (req, res) {
    try {
        ...
        // queryAllCars transaction - requires no arguments, ex: ('queryAllCars')
        var result = [];
        const collectionfile = fs.readFileSync('../../Risk_collection_config.json', 'utf8')
...
app.get('/api/monitor/risks', async function (req, res) {
    try {
        ...
        for (var i = 0; i < collection.length; i++) {
            // console.log(collection[i]['name']);
            const r = await contract.evaluateTransaction('MonitorRisksByRange', 'RISK1', '', collection[i]['name']);
        ...
        // const result2 = await contract.evaluateTransaction('MonitorRisksByRange', 'RISK1','', 'org1-rm-com');
        // const result3 = await contract.evaluateTransaction('MonitorRisksByRange', 'RISK1','', 'org2-rm-com');
...
// app.get('/api/history/risk/:risk_index', async function (req, res) {
app.get('/api/history/:risk_index', async function (req, res) {
    try {
        ...
        // queryCar transaction - requires 1 argument, ex: ('queryCar', 'CAR4')
        // queryAllCars transaction - requires no arguments, ex: ('queryAllCars')
        const result = await contract.evaluateTransaction('GetRiskHistory', req.params.risk_index);
        console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
        // res.status(200).json({response: result.toString()});
        res.json(JSON.parse(result.toString()));
