...
app.get('/api/risks', async function (req, res) {
    try {
        ...
        const identity = await wallet.get('riskmanager2');
        if (!identity) {
        ...
        await gateway.connect(ccp, { wallet, identity: 'riskmanager2', discovery: { enabled: true, asLocalhost: true } });
...
app.post('/api/mitigations', async function (req, res) {
    try {
        ...
        // createCar transaction - requires 5 argument, ex: ('createCar', 'CAR12', 'Honda', 'Accord', 'Black', 'Tom')
        // changeCarOwner transaction - requires 2 args , ex: ('changeCarOwner', 'CAR10', 'Dave')
        // await contract.submitTransaction('CreateMitigation','mit5','helo','riskmanager2','now','RISK1:hi:0.3:0.2;');
...
app.get('/api/monitor/risks', async function (req, res) {
    try {
        ...
        // const result2 = await contract.evaluateTransaction('MonitorRisksByRange', 'RISK1','', 'org2-rm-com');
